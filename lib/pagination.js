...
const partsGetters = [
  {
    ...
    get: query => ({ text: '<<', query: { ...query, current: 1 } }),
  },
  {
    condition: (current, countOfPages, windowMaxSize) => (countOfPages > windowMaxSize
      && current > 1),
    get: (query, current) => ({ text: '<', query: { ...query, current: current - 1 } }),
  },
  {
    condition: () => true,
    get: (query, current, countOfPages, windowMaxSize) => {
      ...
      for (let i = indexes.min; i <= indexes.max; i += 1) {
        items.push({ text: `${i}`, query: { ...query, current: i }, active: i === current });
      }
      return items;
    },
  },
  ...
  },
  {
    ...
    get: (query, current, countOfPages) => ({ text: '>>', query: { ...query, current: countOfPages } }),
  },
];
