...
const findByIdMakers = {
  User: (Model, id) => Model.findByPk(id),
  TaskStatus: (Model, id) => Model.findByPk(id),
  Task: (Model, id) => Model.scope('full').findByPk(id),
};
...
export const isValidId = async (id, Model, ctx) => {
  const result = await Model.findByPk(Number(id));
  if (!result) {
...
const queryDataHandlers = {
  ...
  tags: {
    getValue: value => (_.isArray(value) ? value.map(item => Number(item)) : [Number(value)]),
    getScope: ids => (ids.length > 0 ? { method: ['filterByTagsIds', ids] } : null),
  },
};
